...
export default function Dashboard() {
  ...
  function Task({ taskKey, label, badge, badgeClass }: {
    ...
    return (
      <div
        className="flex justify-between items-center py-2.5 border-b border-[var(--c-border)] cursor-pointer last:border-0"
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className={`text-sm ${done ? 'line-through text-[var(--c-muted)]' : ''}`}>
            {label}
          </span>
        </div>
        <span className={`badge ${done ? 'badge-green' : badgeClass}`}>
          {done ? 'Ferdig' : badge}
        </span>
      </div>
    )
    ...
    <div className="p-4">
      <div className="flex justify-between items-start mb-5 pt-2">
        <div>
          ...
          <p className="text-xs text-[var(--c-muted)] mt-0.5">Gardshus · Voss, Vestland</p>
        </div>
        ...
        <div className="card" style={{ borderLeft: '3px solid var(--c-accent)' }}>
          ...
          <div className="flex gap-2 items-center">
            ...
            <span className="text-xs text-[var(--c-muted)]">
              {daysUntil(nextBooking.check_in)} dagar igjen
            </span>
          </div>
        </div>
      ) : (
      ...
      <div className="card">
        {lowStock.map((item) => (
          <Task
            key={item.id}
            taskKey={'stock_' + item.id}
            label={'Fyll på ' + item.name.toLowerCase()}
            badge="Lager"
        ...
        {nextBooking && daysUntil(nextBooking.check_in) <= 2 && (
          <Task
            taskKey={'turnover_' + nextBooking.id}
            label={'Klargjere til ' + nextBooking.guest_name}
            badge="Turnover"
